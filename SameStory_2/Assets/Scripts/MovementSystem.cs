...
public class MovementSystem : MonoBehaviour
{
    ...
    private void CalcualteRange(Unit selectedUnit, HexGrid hexGrid)
    {
    ...
    public void ShowPath(Vector3Int selectedHexPosition, HexGrid hexGrid)
    {
        if (movementRange.GetRangePositions().Contains(selectedHexPosition))
        {
            ...
            foreach (Vector3Int hexPosition in currentPath)
            {
                hexGrid.GetTileAt(hexPosition).HiglightPath();
            }
        }
    }
    public void MoveUnit(Unit selectedUnit, HexGrid hexGrid)
    {
        Debug.Log("Moveing Unit " + selectedUnit.name);
    ...
    internal void ShowPath(object selectedHexPosition, HexGrid hexGrid)
    {
